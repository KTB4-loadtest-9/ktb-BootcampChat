...
```bash
pnpm run test:medium  # 200명, 배치 접속
```
...
```bash
pnpm run test:rampup  # 500명, ~100개 방까지 점진적 증가, 60초 유지
```
...
- 실행 중인 KTB Chat 백엔드 서버
  - REST API (기본: http://localhost:5001)
...
| `--duration` | `-d` | 테스트 지속 시간 (초, 0 = 메시지 소진까지) | 0 |
...
### Rate Limiting
- REST API: `GET /api/rooms` 엔드포인트에만 적용, IP당 10000회/30초 제한
- Socket.IO: `chatMessage` 이벤트는 유저당 10000회/분 제한
- 현재 기본값으로는 부하기가 rate limit에 실질적으로 걸리지 않음
- 한도 변경 여부 확인: `rg -n "@RateLimit|checkRateLimit" apps/backend/src/main/java`

### 세션 TTL
- 세션 만료 시간: 30분 (`Session.java:24` `SESSION_TTL = "30m"`)
- 30분을 넘는 장시간 테스트는 재인증 로직이 필요함
...
### 소켓 이벤트 계약
- Socket.IO 이벤트명의 단일 소스: `apps/backend/src/main/resources/static/api/docs/socketio/asyncapi.yaml`
- 부하기 스크립트는 이벤트명 문자열 리터럴 대신 `loadtest/socket-contract.js` 상수만 사용 (Socket.IO 예약 이벤트 `connect`, `disconnect`, `connect_error` 제외)
- 계약 드리프트는 `pnpm test`(jest)가 정적으로 검사 — 서버 기동 불필요
- ⚠️ 시나리오 구성(이벤트 종류, 비율)이 변경되면 기존 baseline 측정 결과와 비교 불가 — baseline 재측정 필요
...
- **1초마다 새로운 채팅방 1개 생성**
- 각 채팅방에 2-5명(설정 가능)의 사용자가 랜덤하게 로그인
- 각 사용자는 방에 입장하자마자 메시지 전송 시작
- 최대 사용자 수(기본 500명)까지 지속적으로 증가
- **결과: ~100-250개의 채팅방에 500명 분산** (임계점 테스트에 최적)
- 최대 도달 후 설정된 시간(기본 60초) 동안 부하 유지
- 유지 시간 종료 후 모든 사용자 일시 종료

### 사용법

```bash
# 기본 실행 (최대 500명, 60초 유지)
...
# pnpm 스크립트 사용
pnpm run test:rampup         # 기본 설정
pnpm run test:rampup:small   # 작은 규모 (200명)
pnpm run test:rampup:large   # 대규모 (1000명)
```
...
| `--min-users-per-second` | - | 초당 추가할 최소 사용자 수 | 2 |
| `--max-users-per-second` | - | 초당 추가할 최대 사용자 수 | 5 |
| `--sustain-duration` | `-s` | 최대 부하 유지 시간 (초) | 60 |
| `--message-interval-min` | - | 메시지 전송 최소 간격 (밀리초) | 500 |
| `--message-interval-max` | - | 메시지 전송 최대 간격 (밀리초) | 3000 |
| `--api-url` | - | REST API URL | http://localhost:5001 |
| `--socket-url` | - | Socket.IO URL | http://localhost:5002 |
| `--room-id` | - | 채팅방 ID (없으면 자동 생성) | null |
...
**예시 (500명 목표):**
- 0초: Room #1 생성 → 3명 로그인 → Room #1 입장 (총: 1방, 3명)
- 1초: Room #2 생성 → 5명 로그인 → Room #2 입장 (총: 2방, 8명)
- 2초: Room #3 생성 → 2명 로그인 → Room #3 입장 (총: 3방, 10명)
- ...
- 약 100-250초 후: ~100-250개 방, 500명 도달
- 60초간 유지 후 일시 종료
