...
public class RateLimitService {
    ...
    public RateLimitCheckResult checkRateLimit(String _clientId, int maxRequests, Duration window) {
        ...
        try {
            Counter counter = rateLimitStore.increment(actualClientId, windowSeconds);
            long retryAfterSeconds = Math.max(1L, counter.resetEpochSeconds() - nowEpochSeconds);

            if (counter.count() > Math.max(1, maxRequests)) {
                return RateLimitCheckResult.rejected(
                        maxRequests, windowSeconds, counter.resetEpochSeconds(), retryAfterSeconds);
            }

            int remaining = (int) Math.max(0L, maxRequests - counter.count());

            return RateLimitCheckResult.allowed(
                    maxRequests, remaining, windowSeconds,
                    counter.resetEpochSeconds(), retryAfterSeconds);
        } catch (Exception e) {
